...
_subscription_download() {
    ...
    if [ "$size" -gt "$SUB_MAX_BYTES" ]; then
        log_error "[subscription] subscription exceeds 5MB size limit"
        rm -f "$tmpfile"
        return 1
    fi
    return 0
...
_subscription_validate() {
    file=$1
    [ -s "$file" ] || {
        log_error "[subscription] downloaded file is empty"
        return 1
    }
    grep -q '^proxies:' "$file" || {
        log_error "[subscription] no proxies found in subscription (missing 'proxies:' key)"
        return 1
    }
    grep -qE '^[[:space:]]*-[[:space:]]*name:|^[[:space:]]*-[[:space:]]*\{name:' "$file" || {
        log_error "[subscription] no proxy entries found in subscription"
        return 1
    }
...
EOF
    ...
    rm -f "$test_cfg"
    if [ "$ret" -ne 0 ]; then
        log_error "[subscription] mihomo -t validation failed: $(printf '%s' "$err" | head -3)"
        return 1
    fi
...
_subscription_apply() {
    ...
    chmod 600 "$1"
    mv "$1" "$cur" || {
        log_error "[subscription] atomic apply failed"
        return 1
    }
...
subscription_update() {
    ...
    if [ -z "$url" ]; then
        log_warn "[subscription] no subscription URL configured"
        release_lock
        return 1
    fi
    validate_url "$url" || {
        log_error "[subscription] URL must be HTTPS"
        release_lock
        return 1
    }
    ...
    _subscription_download "$url" "$tmpfile" || {
        release_lock
        return 1
    }
...
subscription_restore() {
    ...
    cur="$SUB_DIR/current.yaml"
    [ -f "$bak" ] || {
        log_error "[subscription] no backup available"
        return 1
    }
