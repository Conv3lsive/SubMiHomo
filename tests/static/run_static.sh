...
if command -v shellcheck >/dev/null 2>&1; then
    ...
    if shellcheck --shell=sh -x --source-path=files/usr/lib/submihomo \
        ...
        note "FAIL: shellcheck warnings present"
        FAIL=1
    fi
...
if command -v shfmt >/dev/null 2>&1; then
    for f in $SHELL_FILES files/etc/init.d/submihomo; do
        if shfmt -ln posix "$f" >/dev/null 2>&1; then :; else
            note "FAIL: shfmt parse error in $f"
            FAIL=1
        fi
    done
...
if command -v luac >/dev/null 2>&1; then
    if luac -p files/usr/lib/rpcd/submihomo 2>/dev/null; then
        ...
        note "FAIL: rpcd plugin Lua syntax error"
        FAIL=1
    fi
else
...
if grep -nE '\bgoto\b|\bbit32\.|\bmath\.type\b|[0-9] // [0-9]|[a-z] << [0-9]|[a-z] >> [0-9]' \
    ...
    note "FAIL: Lua 5.2+ construct found (breaks OpenWrt Lua 5.1)"
    FAIL=1
else
...
for j in files/usr/share/luci/menu.d/luci-app-submihomo.json \
    ...
    if command -v jq >/dev/null 2>&1; then
        if jq empty "$j" 2>/dev/null; then note "PASS: $j"; else
            note "FAIL: $j"
            FAIL=1
        fi
...
if command -v node >/dev/null 2>&1; then
    for js in files/htdocs/luci-static/resources/view/submihomo/*.js; do
        if node --check "$js" 2>/dev/null; then
            note "PASS: $(basename "$js")"
        else
            note "FAIL: $(basename "$js")"
            FAIL=1
        fi
    done
else
...
sec "RESULT"
if [ "$FAIL" -eq 0 ]; then
    note "LAYER 1 STATIC ANALYSIS: PASS"
    exit 0
else
    note "LAYER 1 STATIC ANALYSIS: FAIL"
    exit 1
fi
